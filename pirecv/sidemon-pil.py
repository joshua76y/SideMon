...
def pg_clash(cl):
    ...
    card(d, 10, y0, cw2, 46)
    d.text((20, y0+4), "NODE", fill=TX3, font=F["r10"])
    d.text((20, y0+20), node, fill=TX, font=F["b18"])

    card(d, 20+cw2, y0, cw2, 46)
    mode = cl.get("mode", "Rule").capitalize()
    d.text((28+cw2, y0+4), "MODE", fill=TX3, font=F["r10"])
    d.text((28+cw2, y0+20), mode, fill=AC["codex"], font=F["b18"])
    ...
    d.text((20, y1+4), "TRAFFIC", fill=TX3, font=F["r10"])
    if tu and tt:
        d.text((20, y1+20), f"{tu} / {tt}", fill=TX, font=F["b18"])
        tp = 0
        try:
            parts = tu.split()
            if parts:
        ...
        d.text((20, y1+20), "N/A", fill=TX3, font=F["r18"])
    ...
    d.text((18, y2+22), exp, fill=AC["wthr"], font=F["b14"])

    card(d, 20+cw3, y2, cw3, 46)
    d.text((28+cw3, y2+4), "VERSION", fill=TX3, font=F["r10"])
    ver = cl.get("version", "")[:8] or "?"
    d.text((28+cw3, y2+22), ver, fill=TX2, font=F["b14"])
    ...
    d.text((30+2*cw3+(cw3-tc)//2, y2+18), conns, fill=ac, font=F["b20"])
...
def pg_codex(cx):
    ...
    for cx_pos, pct_key, label in rings:
        ...
        d.text((cx_pos - tw//2, ring_y - 14), pct_text, fill=TX, font=F["b26"])
        # Label below ring
        lw = d.textlength(label, font=F["b16"])
        d.text((cx_pos - lw//2, ring_y + radius + 14), label, fill=ac, font=F["b16"])

    # Bottom info card: reset times with larger font
    by = 232
    card(d, 10, by, W-20, 48)
    d.text((20, by+2), "RESET 5H", fill=TX3, font=F["r10"])
    d.text((20, by+16), cx.get("reset_5h","?"), fill=ac, font=F["b18"])
    d.text((W//2+4, by+2), "RESET 7D", fill=TX3, font=F["r10"])
    d.text((W//2+4, by+16), cx.get("reset_7d","?"), fill=AC["wthr"], font=F["b18"])
